...
export async function getLikeCounts(cafeIds: string[]): Promise<Record<string, number>> {
  ...
  for (const row of data ?? []) counts[row.cafe_id] = (counts[row.cafe_id] ?? 0) + 1;
  return counts;
}

/** Whether the current user has liked a cafe */
export async function getUserLike(userId: string, cafeId: string): Promise<boolean> {
  const supabase = createClient();
  const { data } = await supabase
    .from("likes")
    .select("id")
 ...
 */
export async function getLikersForCafe(cafeId: string): Promise<LikeWithProfile[]> {
  const supabase = createClient();
  const { data } = await supabase
    .from("likes")
